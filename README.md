...
Решение задания представлено в двух файлах (папка features): сценарий ``web_page_test.feature`` и шаги выполнения ``web_ui_steps.rb`` (папка step_definitions).

### 2) Реализация шагов удаления и изменения в API.
Решение задания представлено в двух файлах (папка features): сценарий ``rest_api_test.feature`` и шаги выполнения ``rest_tests_steps.rb`` (папка step_definitions).    
